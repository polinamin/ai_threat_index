...
combinedtext_ = [(text_tasks), (text_jobs), (text_tools)]#[text_process(text_tasks),text_process(text_job), text_process(text_tools)]
...
if st.button('Submit'):
  if len(text_jobs)==len(text_tasks) and len(text_jobs)==len(text_tools) and len(text_jobs) == len(text_skills):
...
for i in range(len(tools_list)):
    ...
    if len(difflib.get_close_matches(j,Xstreamlit_tools.columns)) != 0:
        k = difflib.get_close_matches(j,Xstreamlit_tools.columns, cutoff=0.8)[0]
